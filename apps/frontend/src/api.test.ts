...
describe('createApiClient: リクエストの形', () => {
  it('createPayload: POST /api/payload に、暗号文・IV・種別・TTL だけをヘッダー/本文で送る', async () => {
    ...
    const result = await client.createPayload({ encryptedData, iv: IV, type: 'file', ttlSeconds: 3600 });
  ...
  it('getMeta は GET …/meta、consume は POST …/consume（本文なし）', async () => {
    ...
    const consumed = await client.consume(ID);
...
describe('createApiClient: 鍵が URL に入り込まない', () => {
  it('形式の正しくない ID（# ? / . を含む・長さ違い）は、リクエストを作る前に拒否する', async () => {
    ...
    for (const id of bad) {
      ...
      assert.equal((await errorOf(client.consume(id))).code, 'not_found', id);
    }
...
describe('createApiClient: エラーの分類', () => {
  ...
  it('接続できない（fetch が reject）は network。メッセージは固定で、URL・ID を含まない', async () => {
    ...
    const error = await errorOf(client.consume(ID));
  ...
  it('consume の本文の読み取りが途中で失敗したら network', async () => {
    ...
    assert.equal((await errorOf(client.consume(ID))).code, 'network');
  });
});
...
describe('createApiClient: サーバーの応答は信頼せず検証する', () => {
  ...
  for (const [index, body] of badCreate.entries()) {
    it(`作成の応答が不正（${index}）: ${JSON.stringify(body)} → invalid_response`, async () => {
      ...
      const error = await errorOf(client.createPayload({ encryptedData: new ArrayBuffer(20), iv: IV, type: 'text', ttlSeconds: 3600 }));
  ...
  it('consume の IV ヘッダーが無い・不正・12 バイトでない場合は invalid_response', async () => {
    ...
    for (const iv of [null, '', '@@@@', 'AAAA+/AAAAAAAAA=', wrongLength]) {
      ...
      assert.equal((await errorOf(client.consume(ID))).code, 'invalid_response', String(iv));
    }
  });
});
